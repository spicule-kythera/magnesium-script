...
public class Alert extends Expression {
  ...
  public Alert parse(Map<String, Object> tokens) throws InvalidExpressionSyntax {
    ...
    try {
      ...
      throw new InvalidExpressionSyntax("alert", "Invalid action type: `" + actionToken
          + "`! Token must be one of the following: " + Arrays.toString(Action.values()));
    }

    // Process `input` field
    if (action.equals(Action.KEYS)) {
      if (!tokens.containsKey("input")) {
        throw new InvalidExpressionSyntax("alert",
            "`keys` action requires `input` field containing string of input to send!");
      }
    ...
    try {
      ...
      throw new InvalidExpressionSyntax("alert",
          "Invalid `timeout` value: `" + timeoutToken + "`! Value must be whole integer!`");
    }
  ...
  public void execute() {
    try {
      ...
      // alert = new WebDriverWait(driver,
